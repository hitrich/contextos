...
function memoryForm(memory = null) {
  ...
  inspector.close();
  openModal(`<form id="memory-form" data-id="${esc(memory?.id || '')}">${modalHead(memory?'Edit memory':'Make a little room for context.',memory?'Every edit leaves a version you can come back to.':'A thought today. A useful starting point tomorrow.')}<div class="modal-body">${formError()}<div class="form-grid">
  ...
  modal.querySelector('#memory-form').addEventListener('submit', async event => {
    ...
    await submit(form, async()=>{ const result = await api(memory?`/memories/${encodeURIComponent(memory.id)}`:'/memories',memory?'PUT':'POST',input); modal.close(); await refresh(); toast(memory?'New version saved. Your history is intact.':'Memory saved. A little more context to come back to.'); await openMemory(result.id); });
  });
}
